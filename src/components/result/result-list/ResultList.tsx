...
const ResultList = ({ type }: { type: interviewType }) => {
  ...
  {data?.pages.map((page, i) =>
    page?.data?.interviewList?.content?.length ? (
      page.data.interviewList.content.map((interview: IResultListItem) =>
      ...
      <div key={i} className="text-center py-10 text-[#888]">불러올 결과가 없습니다</div>
    )
  )}
